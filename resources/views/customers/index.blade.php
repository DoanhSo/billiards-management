...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h2 class="h3 mb-1">Quản lý khách hàng</h2>
        <p class="text-muted mb-0" style="font-size: 14px;">Danh sách tài khoản khách hàng trong hệ thống</p>
    </div>
...
<div class="row g-3 mb-4">
    <div class="col-6 col-md-4">
        <x-stat-card title="Tổng khách hàng" :value="$users->total()" icon="people" color="success" />
    </div>
    <div class="col-6 col-md-4">
        <x-stat-card title="Đang hoạt động" :value="$users->where('status', true)->count()" icon="check-circle" color="primary" />
    </div>
    <div class="col-6 col-md-4">
            ...
            <tr>
                    ...
                    <div class="d-flex align-items-center gap-3">
                        ...
                        @else
                            <div class="rounded-circle d-flex align-items-center justify-content-center text-white fw-bold"
                                 style="width: 40px; height: 40px; background: linear-gradient(135deg, #16a34a, #059669); font-size: 16px;">
                                {{ strtoupper(mb_substr($user->name, 0, 1)) }}
                            </div>
                        @endif
                        <div>
                            <span class="fw-semibold d-block">{{ $user->name }}</span>
                            <x-badge type="success">Khách hàng</x-badge>
                        </div>
                    </div>
                </td>
                ...
                <td class="text-end">
                    <div class="d-flex justify-content-end gap-1">
                        ...
                        {{-- Sửa --}}
                        <a href="{{ route('customers.edit', $user->id) }}" class="btn btn-sm btn-outline-primary" title="Chỉnh sửa">
                            <i class="bi bi-pencil-square"></i>
                        </a>
                        {{-- Xóa --}}
                        <form action="{{ route('customers.destroy', $user->id) }}" method="POST" class="d-inline"
                              data-name="{{ $user->name }}"
                              onsubmit="return confirm('Xóa tài khoản khách hàng ' + this.getAttribute('data-name') + '? Hành động này không thể hoàn tác.');">
                            @csrf
                            @method('DELETE')
        ...
        @empty
            <tr>
                <td colspan="6" class="text-center py-5">
                    ...
                    <p class="text-muted mt-3 mb-0">Không tìm thấy khách hàng nào.</p>
                    @if($search || $status !== '')
