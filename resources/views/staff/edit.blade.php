            ...
            <form action="{{ route('staff.update', $user->id) }}" method="POST" enctype="multipart/form-data" class="p-3 row g-3" novalidate>
                    ...
                    <div class="d-flex align-items-center gap-3">
                        ...
                        @else
                            <div class="rounded-circle d-flex align-items-center justify-content-center text-white fw-bold shadow-sm"
                                 style="width: 64px; height: 64px; background: linear-gradient(135deg, #2563eb, #7c3aed); font-size: 24px;">
                                {{ strtoupper(mb_substr($user->name, 0, 1)) }}
                            </div>
                        @endif
                        <div>
                            <h6 class="mb-0">{{ $user->name }}</h6>
                            <small class="text-muted">{{ $user->email }} · <x-badge type="primary">Nhân viên</x-badge></small>
                        </div>
                    </div>
                </div>
                <hr class="my-2">
                ...
                <div class="col-12">
                    <div class="alert alert-info d-flex align-items-center gap-2 mb-0" style="font-size: 13px;">
                        ...
                        <span>Mật khẩu không thể thay đổi tại đây. Nhân viên tự đổi mật khẩu trong phần <strong>Đổi mật khẩu</strong>.</span>
                    </div>
                </div>

                <div class="col-12 mt-4 d-flex gap-2">
                    <x-button type="submit" variant="primary" icon="check-lg">Cập nhật nhân viên</x-button>
