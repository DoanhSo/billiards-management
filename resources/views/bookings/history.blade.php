...
<div class="container-fluid px-0">
    ...
    <div class="filter-bar mb-4">
        <div class="d-flex align-items-center gap-3 flex-wrap">
            <div style="font-size: 0.8rem; color: var(--text-muted-c); font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em;">
                Tổng cộng:
            </div>
            <span style="background: var(--info-glow); color: var(--info); border: 1px solid rgba(56,189,248,0.3); padding: 4px 12px; border-radius: 20px; font-size: 0.8rem; font-weight: 700;">
                <i class="bi bi-check2-all me-1"></i>Hoàn thành: {{ $bookings->where('status','COMPLETED')->count() }}
            </span>
            <span style="background: var(--danger-glow); color: var(--danger); border: 1px solid rgba(248,113,113,0.3); padding: 4px 12px; border-radius: 20px; font-size: 0.8rem; font-weight: 700;">
                <i class="bi bi-x-circle me-1"></i>Đã hủy: {{ $bookings->where('status','CANCELLED')->count() }}
            </span>
            <span style="margin-left: auto; color: var(--text-muted-c); font-size: 0.8rem;">
                ...
                <table class="table table-hover mb-0">
                    ...
                    <tbody>
                        @forelse($bookings as $index => $booking)
                            <tr>
                                {{-- STT --}}
                                <td style="padding-left: 20px !important; color: var(--text-muted-c); font-weight: 600;">
                                ...
                                <td>
                                    <div class="d-flex align-items-center gap-2">
                                        <div class="avatar-circle" style="background: rgba(148,163,184,0.1); color: var(--text-secondary);">
                                            {{ strtoupper(substr($booking->user->name, 0, 1)) }}
                                        </div>
                                        <div>
                                            <div style="font-weight: 600; font-size: 0.875rem;">{{ $booking->user->name }}</div>
                                            <div class="text-xxs" style="color: var(--text-muted-c);">
                                ...
                                {{-- Bàn chơi --}}
                                <td>
                                    <div style="font-weight: 700; font-size: 0.9rem;">Bàn {{ $booking->billiardTable->table_number }}</div>
                                    <div class="text-xxs" style="color: var(--text-muted-c);">{{ $booking->billiardTable->table_type }}</div>
                                </td>
                                ...
                                <td>
                                    <div style="font-size: 0.875rem;">
                                        ...
                                        <span style="color: var(--text-muted-c); margin: 0 4px;">→</span>
                            ...
                            <tr>
                                <td colspan="8">
                                    <div class="empty-state">
                                        ...
                                        <p>Lịch sử sẽ xuất hiện khi các đặt bàn được hoàn tất hoặc bị hủy.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
