...
<div class="container-fluid px-0" style="max-width: 800px;">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h1 class="h3 mb-0 text-gray-800">Chỉnh sửa Bàn chơi</h1>
            <p class="text-muted mb-0">Thay đổi thông tin bàn chơi: Bàn {{ $table->table_number }}</p>
        </div>
            ...
            <form action="{{ route('tables.update', $table->id) }}" method="POST">
                ...
                <div class="row g-3 mb-3">
                    <div class="col-12 col-md-6">
                        ...
                        <input type="text" id="table_number" name="table_number" class="form-control @error('table_number') is-invalid @enderror" placeholder="VD: 01, A02..." value="{{ old('table_number', $table->table_number) }}" required>
                        @error('table_number')
                ...
                <div class="row g-3 mb-3">
                    <div class="col-12 col-md-6">
                        ...
                        <div class="input-group @error('price_per_hour') is-invalid @enderror">
                            <input type="number" id="price_per_hour" name="price_per_hour" class="form-control @error('price_per_hour') is-invalid @enderror" placeholder="VD: 80000" value="{{ old('price_per_hour', (int)$table->price_per_hour) }}" min="0" step="1000" required>
                    ...
                    <div class="col-12 col-md-6">
                        ...
                        <select id="status" name="status" class="form-select @error('status') is-invalid @enderror">
                            <option value="AVAILABLE" {{ old('status', $table->status) === 'AVAILABLE' ? 'selected' : '' }}>Sẵn sàng (Available)</option>
                            <option value="PLAYING" {{ old('status', $table->status) === 'PLAYING' ? 'selected' : '' }}>Đang chơi (Playing)</option>
                            <option value="RESERVED" {{ old('status', $table->status) === 'RESERVED' ? 'selected' : '' }}>Đã đặt trước (Reserved)</option>
                            <option value="MAINTENANCE" {{ old('status', $table->status) === 'MAINTENANCE' ? 'selected' : '' }}>Bảo trì (Maintenance)</option>
                        </select>
                        @error('status')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                ...
                <div class="mb-4">
                    ...
                    <textarea id="description" name="description" class="form-control @error('description') is-invalid @enderror" rows="4" placeholder="Nhập ghi chú thêm về bàn (Ví dụ: Vị trí góc VIP, bàn mới bọc vải...)">{{ old('description', $table->description) }}</textarea>
                    @error('description')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
